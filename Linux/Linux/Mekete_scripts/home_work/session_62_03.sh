...
1. Create a variable to hold the path of your output file.
    ...
    - Replace the output file path for each command with your variable.
		...
		sysadmin@UbuntuDesktop:~$ touch output.txt
		sysadmin@UbuntuDesktop:~$ out=output.txt 
...
ip addr | grep inet | tail -3 | head -1 >> $out

# permission

sudo find /home -type f -perm 777 >> $out
